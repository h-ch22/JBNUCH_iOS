...
class SportsLocationSelectViewController : UIViewController, CLLocationManagerDelegate{
    var locationManager = CLLocationManager()
    var naverMapView : NMFNaverMapView!
    
    let marker = NMFMarker()
    let receiver : SportsLocationReceiver
    let dragListener : SportsMapDragListener
    let camPositionFormat = "(%.5f, %.5f) / %.2f / %.2f / %.2f"
    
    private let API_KEY = "v60rvp0b1d"
    private let API_SECRET = "LFlzEd2DFKeCTazcXxff7paRv097u0o0ux7vHaPU"
    private let RGC_URL = "https://naveropenapi.apigw.ntruss.com/map-reversegeocode/v2/gc?"
    
    init(receiver : SportsLocationReceiver, dragListener : SportsMapDragListener){
...
extension SportsLocationSelectViewController: NMFMapViewCameraDelegate {
