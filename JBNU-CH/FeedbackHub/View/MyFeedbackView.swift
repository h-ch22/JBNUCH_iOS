...
struct MyFeedbackView: View {
    ...
    private var filtertedList : [FeedbackHubDataModel]{
        if searchText.isEmpty{
            return helper.myfeedbackList
        }
        
        else{
            return helper.myfeedbackList.filter{$0.title.localizedCaseInsensitiveContains(searchText) as! Bool || $0.contents.localizedCaseInsensitiveContains(searchText) as! Bool}
        }
    }
            ...
            VStack{
                List{
                    ForEach(filtertedList, id : \.self){ index in
                        NavigationLink(destination : FeedbackHubDetailView(data : index, type : "User", userManagement: userManagement, helper : helper)){
                            FeedbackHubListModel(data : index, type : "User")
                        }
                    }
                }.refreshable{
