...
struct PetitionDetailView_Beta: View {
            ...
            ZStack{
                ...
                VStack{
                    ...
                    Group{
                        ...
                        .controlSize(.large)
                        .isHidden(self.userInfo.userInfo?.uid == AES256Util.decrypt(encoded: self.item.author ?? "") || helper.recommenders.contains(where: {$0.uid == self.userInfo.userInfo?.uid}) || item.status == .Cancled || item.status == .Finish || item.status == .Cancled)
