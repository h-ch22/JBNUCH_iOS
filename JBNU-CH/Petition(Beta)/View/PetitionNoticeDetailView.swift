//
//  PetitionNoticeDetailView.swift
//  JBNU-CH
//
...
struct PetitionNoticeDetailView: View {
    ...
    var body: some View {
        ScrollView{
            ...
            .sheet(isPresented : $showEditWindow){
                editPetitionNoticeView(helper : helper, userManagement: userManagement, data : data)
            }
            ...
            .alert(isPresented : $showAlert){
                ...
                case .confirm :
                    return Alert(title: Text("공지사항 제거"), message: Text("공지사항을 제거하시겠습니까?"), primaryButton: .default(Text("예")){
                        ...
                        helper.removeNotice(id : data.id ?? ""){result in
            ...
            .fullScreenCover(isPresented : $showSelectLanguage, content : {
                PetitionTranslateLanguageSelectionView(data : self.data, helper : self.helper).onAppear{
            ...
            .onAppear{
                ...
                if data.imageIndex ?? 0 > 0{
                    helper.downloadNoticeImage(id: data.id!, imageIndex: data.imageIndex ?? 0){result in
                        guard let result = result else{return}
